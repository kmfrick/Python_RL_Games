...
Original file is located at
    https://colab.research.google.com/drive/1P7QAOwGFxe2JDTMB_69mwHlKKpZxgxzH
"""
...
class static_game():
	...
	def __init__(self, game_mode):
		if game_mode == "prisoner_dilemma":
			self.payoffs = [[(5, 5), (-5, 10)],
							[(10, -5), (0, 0)]]		# cooperate or defect
			self.n_actions = 2
		...
		self.n_states = self.n_actions ** (n_agents * memory)
		self.initial_state = np.full(memory, 0)

		self.one_memory_n_states = self.n_actions ** n_agents	# memory_lenght = 1
		self.one_memory_states = list(range(0, self.one_memory_n_states))
		self.one_memory_next_state = np.array_split(self.one_memory_states, self.n_actions)

	def reset_initial_state(self):
		self.initial_state = np.random.choice(self.one_memory_states, memory)
		return self.initial_state

	def get_payoffs(self, action1, action2):
		return self.payoffs[action1][action2]

	def get_next_state_vector(self, state, action1, action2):
		if memory == 0:
			return 0
		elif memory == 1:
			return self.one_memory_next_state[action1][action2]
		elif memory > 1:
			state[0:memory-1] = state[1:memory]
			state[memory-1] = game.one_memory_next_state[action1][action2]
			return state   

	def get_state_number(self, state): 
		return np.dot(state, [self.one_memory_n_states**(x) for x in range(memory-1,-1,-1)]).item()
...
def training(game, player1, player2, episode_max = 10**4):
	...
	state_vector = game.reset_initial_state()
	...
	freq_states = np.zeros(game.one_memory_n_states)

	# loop over episodes
	for episode in range(episode_max):
		# get state number from state vector
		state_number = game.get_state_number(state_vector)
  
    # get epsilon-greedy action given current state
		action1 = player1.get_action(state_number)
		action2 = player2.get_action(state_number)

		# collect instantaneous reward and observe next period state 
		reward1, reward2 = game.get_payoffs(action1, action2)
		next_state_vector = game.get_next_state_vector(state_vector, action1, action2)

		# get next state number from next state vector
		next_state_number = game.get_state_number(next_state_vector)

		# update q value for the current state according to learning rate, reward and continuation value
		player1.update_q(state_number, action1, reward1, next_state_number)
		player2.update_q(state_number, action2, reward2, next_state_number)

		# set state vector to next state vector and iterate
		...
		freq_states[game.one_memory_next_state[action1][action2]] += 1
...
for epoch in range(10**5):
	...
	for s in range(0, game.n_states):
